...
try {
    ...
    $mail->Body    = ('Đơn hàng gần đây của bạn đã bị huỷ do hết hàng.');
    // $mail->AltBody = 'This is the body in plain text for non-HTML mail clients';

    if ($mail->send()) {
        echo '<script>alert("Đơn hàng đã bị huỷ")</script>';
        echo '<script>window.open("../view_orders.php","_self")</script>';
    } else {
        echo "Không thể gửi email";
    }
} catch (Exception $e) {
    echo "Không thể gửi email lúc này. Lỗi: {$mail->ErrorInfo}";
}
